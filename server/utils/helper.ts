...
function convertBox(
  initBox: number[],
  imageDim,
  initFormat: 'CCWH' | 'XYWH' | 'XYXY' = 'XYXY',
  initNormalized = false,
  finalFormat: 'CCWH' | 'XYWH' | 'XYXY' = 'CCWH',
...
async function resize(image: Buffer, dim: [number, number], background_color = [255, 255, 255]): Promise<[Buffer, [number, number]]> {
...
async function reshape(buffer: Buffer, [width, height]: [number, number]) {
  ...
  for (let i = 0; i < height; i++) {
    ...
    for (let j = 0; j < width; j++) {
      const pixel = []
      for (let k = 0; k < channels; k++) pixel.push(flatArray[(i * width + j) * channels + k])
      row.push(pixel)
    }
    shapeArray.push(row)
  }
